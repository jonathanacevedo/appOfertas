...
public class EliminarApiController implements EliminarApi {
    ...
    public ResponseEntity<JsonApiBodyResponseSuccess> eliminarDelete(@ApiParam(value = "body" ,required=true )  @Valid @RequestBody JsonApiBodyRequestDelete body) {
        ...
        if (accept != null && accept.contains("application/json")) {
            try {
            	RegistrarRequest superA = repo.findOne(body.getPersona().get(0).getParametro());
            	if(superA.getRol().equals("superAdmin")) {
            		//RegistrarRequest usuario = repo.findOne(body.getPersona().get(0).getId());
		            		repo.delete(repo.findOne(body.getPersona().get(0).getId()));
            	} else {
            		System.out.println("Usted no es super administrador");
            	}

                return new ResponseEntity<JsonApiBodyResponseSuccess>(objectMapper.readValue("{  \"estado\" : \"Usuario registrado correctamente\",  \"id\" : \"id\",  \"nombre\" : \"nombre\"}", JsonApiBodyResponseSuccess.class), HttpStatus.NOT_IMPLEMENTED);
            } catch (IOException e) {
