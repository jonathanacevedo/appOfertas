...
public class RegistrarApiController implements RegistrarApi {
    ...
    public ResponseEntity<JsonApiBodyResponseSuccess> registrarPost(@ApiParam(value = "body" ,required=true )  @Valid @RequestBody JsonApiBodyRequest body) {
        String accept = request.getHeader("Accept");
        if (accept != null && accept.contains("application/json")) {
            try {
            	RegistrarRequest negocio = body.getNegocio().get(0);
            	repo.save(body.getNegocio().get(0));
                return new ResponseEntity<JsonApiBodyResponseSuccess>(objectMapper.readValue("{  \"estado\" : \""+negocio.getNit()+"\",  \"id\" : \""+negocio.getTipo()+"\",  \"nombre\" : \""+negocio.getNombre()+"\"}", JsonApiBodyResponseSuccess.class), HttpStatus.ACCEPTED);
            } catch (IOException e) {
